...
class SSR1Crawler:
    ...
    def parse_detail(
        ...
        cover_pattern: re.Pattern[str] = re.compile(
            r'<img[^>]*?src="([^"]*)"[^>]*?class="cover"'
        )
        name_pattern: re.Pattern[str] = re.compile(
            r'<h2[^>]*?class="m-b-sm">(.*?)</h2>', re.S
        )
        categories_pattern: re.Pattern[str] = re.compile(
            r"<button[^>]*?category.*?<span>(.*?)</span>.*?</button>", re.S
        )
        published_at_pattern: re.Pattern[str] = re.compile(
            r"(\d{4})-(\d{2})-(\d{2})\s*?上映"
        )
        drama_pattern: re.Pattern[str] = re.compile(
            r"剧情简介</h3>.*?<p.*?>(.*?)</p>", re.S
        )
        score_pattern: re.Pattern[str] = re.compile(
            r'<p[^>]*?class="score[^"]*">(.*?)</p>', re.S
        )
        ...
        for url in url_list:
            ...
            try:
                ...
                published_at = {"month": None, "day": None, "year": None}
            except TypeError:
                published_at = {"month": None, "day": None, "year": None}
    ...
    def save_data(
        self,
        film_dict: dict[str : dict[str : str | float | None]] | None = None,
        dir_or_file_name: str | None = None,
        auto_replace_illegal_character: bool = True,
        ...
        if multiple_files:
            ...
            for film_url, film_detail in film_dict.items():
                ...
                with open(
                    f"{dir_or_file_name}/{file_name}", "w", encoding="utf-8"
                ) as f:
